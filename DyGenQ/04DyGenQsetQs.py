...
def argparser():
    ...
    parser.add_argument('--prompt', default='./promptEn/setCQ.txt', type=str)
    parser.add_argument('--dataset', default='./output/KNexplain.json', type=str)
    parser.add_argument('--model', default='Qwen', type=str)
    parser.add_argument('--outputCQ', default='./output/CQ_format.json', type=str)
    parser.add_argument('--fewshot', default='./promptEn/few-shot/setCQ_college_computer_science_dev.txt', type=str)
...
def process(file_path, outputCQ):
    ...
    for index, row in enumerate(tqdm(data, desc="Processing LLM Q&A", ncols=100, unit="item")):
        ...
        for index, kne in enumerate(KNexplain):
            if 'Error' in kne:
                print("kne:",kne)
                print("row:",row)
            prompt = read_txt(configs.prompt).replace("{{KNexplain}}", kne).replace("{{purport}}", purport).replace("{{choiceQ}}", choiceQ).replace("{{few-shot}}", read_txt(configs.fewshot)).replace("{{kn}}", KN[index])
